...
{
    net->valid_steps = option_find_int(options,"valid_steps", 1000);
...
//    fread(l.weights, sizeof(float), num, fp);
    int read_weights = fread(l.weights, sizeof(float), num, fp);
    printf("size of weights read: %d\n", read_weights);
...
int arrays_same(float *f1, float *f2, int n) {
    int same = 1;
    for (int i = 0; i < n; i++) {
        if (f1[i] != f2[i]) {
            same = 0;
...
void compare_convolutional_weights(layer l1, FILE *fp1, layer l2, FILE *fp2)
{
    if(l1.numload) {
        l1.n = l1.numload;
        l2.n = l2.numload;
    ...
    if (l1.n != l2.n) {
        printf("*numload is different*\n");
    }

    int num = l1.c/l1.groups*l1.n*l1.size*l1.size;

    fread(l1.biases, sizeof(float), l1.n, fp1);
    fread(l2.biases, sizeof(float), l2.n, fp2);
    if (! arrays_same( l1.biases, l2.biases, l1.n )) {
        printf("*biases are different*\n");
    }

    if (l1.batch_normalize && (!l1.dontloadscales)){
        fread(l1.scales, sizeof(float), l1.n, fp1);
        fread(l2.scales, sizeof(float), l2.n, fp2);
        if (! arrays_same( l1.scales, l2.scales, l1.n )) {
            printf("*scales are different*\n");
        }

        fread(l1.rolling_mean, sizeof(float), l1.n, fp1);
        fread(l2.rolling_mean, sizeof(float), l2.n, fp2);
        if (! arrays_same( l1.rolling_mean, l2.rolling_mean, l1.n )) {
            printf("*rolling_means are different*\n");
        }

        fread(l1.rolling_variance, sizeof(float), l1.n, fp1);
        fread(l2.rolling_variance, sizeof(float), l2.n, fp2);
        if (! arrays_same( l1.rolling_variance, l2.rolling_variance, l1.n )) {
            printf("*rolling_variances are different*\n");
        }
    }

    fread(l1.weights, sizeof(float), num, fp1);
    fread(l2.weights, sizeof(float), num, fp2);
    if (! arrays_same( l1.weights, l2.weights, num )) {
        printf("*__WEIGHTS__ are different*\n");
    }
}
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    for(i = start; i < net->n && i < cutoff; ++i){
        ...
        if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL){
        	...
        	printf("Loaded conv weights for layer [%d]\n", i);
        }
        if(l.type == CONNECTED){
...
void compare_weights_of_nets(network *net1, network *net2, char *filename1, char *filename2, int start, int cutoff)
{
    fprintf(stderr, "Loading weights from %s and %s...", filename1, filename2);
    fflush(stdout);
    FILE *fp1 = fopen(filename1, "rb");
    if(!fp1) file_error(filename1);
    ...
    int minor;
    int revision;
    fread(&major, sizeof(int), 1, fp1);
    fread(&major, sizeof(int), 1, fp2);
    fread(&minor, sizeof(int), 1, fp1);
    fread(&minor, sizeof(int), 1, fp2);
    fread(&revision, sizeof(int), 1, fp1);
    fread(&revision, sizeof(int), 1, fp2);

    if ((major*10 + minor) >= 2 && major < 1000 && minor < 1000){
        fread(net1->seen, sizeof(size_t), 1, fp1);
        fread(net2->seen, sizeof(size_t), 1, fp2);
    } else {
        int iseen1 = 0;
        int iseen2 = 0;
        fread(&iseen1, sizeof(int), 1, fp1);
        fread(&iseen2, sizeof(int), 1, fp2);
        *net1->seen = iseen1;
        *net2->seen = iseen2;
    }
    // int transpose = (major > 1000) || (minor > 1000);
    ...
    int i;
    for(i = start; i < net1->n && i < cutoff; ++i){
        layer l1 = net1->layers[i];
        layer l2 = net2->layers[i];
        if (l1.dontload) continue;
        if(l1.type == CONVOLUTIONAL || l1.type == DECONVOLUTIONAL){
        	printf("Comparing conv weights for layer [%d]\n", i);
            compare_convolutional_weights(l1, fp1, l2, fp2);
        }
        // if(l.type == CONNECTED){
        //     load_connected_weights(l, fp, transpose);
        // }
