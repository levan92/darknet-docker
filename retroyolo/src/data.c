...
// rambu function. same as get_random_paths() except the last tail paths are given some prob chance of getting in.
// e.g. tail=100, prob=0.5. Then the last 100 paths have a 50% chance of being selected
char **get_random_paths_rs(char **paths, int n, int m, int tail, double prob)
{
    char **random_paths = calloc(n, sizeof(char*));
    int i;
    pthread_mutex_lock(&mutex);
    for(i = 0; i < n; ++i){
    	if (tail && (double)rand() / (double)RAND_MAX < prob) {
            int index = m-tail + (rand()%tail);
            random_paths[i] = paths[index];
    	} else {
            int index = rand()%(m-tail);
            random_paths[i] = paths[index];
    	}
        //if(i == 0) printf("%s\n", paths[index]);
    }
...
{
    FILE *file = fopen(filename, "r");
    // Eugene commented this out. There are some images that exist, but have no corresponding text annotations.
//    if(!file) file_error(filename);
    // Eugene added this if check, and also to return 0 if the if-check fails.
    if (file) {
        float x, y, h, w;
        int id;
        int count = 0;
...
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure, int tail, double prob)
{
    char **random_paths = get_random_paths_rs(paths, n, m, tail, prob);
    int i;
    data d = {0};
    d.shallow = 0;
...
// retroyolo function to load in validation data as-is, no random sampling and no augmentation - just images in network size
data load_data_detection_validation(char **paths, int tid, int n, int w, int h, int boxes, int classes)
{
    int i;
    data d = {0};
    d.shallow = 0;
    ...
    // Not doing augmentation, so the parameters below preserve the boxes
    int flip = 0;
    int dx = 0;
    int dy = 0;
    float sx = 1;
    ...
    int offset = tid * n;
    for(i = 0; i < n; ++i){
        image orig = load_image_color(paths[i + offset], 0, 0);
        image sized = resize_image(orig, w, h);

        d.X.vals[i] = sized.data;

        fill_truth_detection(paths[i + offset], boxes, d.y.vals[i], classes, flip, dx, dy, sx, sy);
        free_image(orig);
    }
    return d;
}
...
void *load_thread(void *ptr)
{
    ...
    if (a.type == OLD_CLASSIFICATION_DATA){
        ...
        *a.d = load_data_detection(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure, a.tail, a.prob);
    } else if (a.type == VALIDATION_DATA){
        *a.d = load_data_detection_validation(a.paths, a.tid, a.n, a.w, a.h, a.num_boxes, a.classes);
