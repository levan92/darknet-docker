...
void compare_network_weights(char *cfg, char *wt1, char *wt2) {
    network *net1 = parse_network_cfg(cfg);
    network *net2 = parse_network_cfg(cfg);
    if(wt1 && wt1[0] != 0 && wt2 && wt2[0] != 0){
        // load_weights(net, weights);
        compare_weights_of_nets(net1, net2, wt1, wt2, 0, net1->n);
    } else {
    	printf("No valid weights given.\n");
    }
...
network *load_network(char *cfg, char *weights, int clear)
{
    ...
    if(weights && weights[0] != 0){
        load_weights(net, weights);
        printf("loaded weights [%s], of n = [%d]\n", weights, net->n);
    } else {
    	printf("No valid weights given.\n");
    }
    if(clear) (*net->seen) = 0;
    if(clear) (*net->valid_timer) = 0;
    return net;
}
...
int time_to_valid(network *net)
{
	int actual_valid_steps = (*net->valid_timer)/(net->batch*net->subdivisions);
    if (actual_valid_steps >= (net->valid_steps)){
    	*net->valid_timer = 0;
    	return 1;
    } else {
    	return 0;
...
network *make_network(int n)
{
    ...
    net->valid_timer = calloc(1, sizeof(size_t));
...
float validate_network_datum(network *net)
{
    net->train = 0;
    forward_network(net);
    float error = *net->cost;
...
float validate_network(network *net, data d)
{
	printf("validate_network: d.X.rows = %d, net->batch = %d\n", d.X.rows, net->batch);
    assert(d.X.rows % net->batch == 0);
    int batch = net->batch;
    int n = d.X.rows / batch;
    ...
    for(i = 0; i < n; ++i){
        get_next_batch(d, batch, i*batch, net->input, net->truth);
        float err = validate_network_datum(net);
        sum += err;
    }
    return (float)sum/(n*batch);
}
...
//        	printf("shortcut: i = %d, l->w = %d, l->out_w = %d\n", i, l.w, l.out_w);
...
void sync_nets(network **nets, int n, int interval)
{
    ...
    *(nets[0]->valid_timer) += interval * (n-1) * nets[0]->batch * nets[0]->subdivisions;

    for (j = 0; j < n; ++j){
        *(nets[j]->seen) = *(nets[0]->seen);
        *(nets[j]->valid_timer) = *(nets[0]->valid_timer);
    }
    for (j = 0; j < layers; ++j) {
