...
{
    int i, j;
    printf(">>>>>>>> validate_classifier_full <<<<<<<<\n");
    network *net = load_network(filename, weightfile, 0);
    set_batch_network(net, 1);
    srand(time(0));
...
// Extracts the epoch number from the weights file
// Assumes that the file name is .../..._[epoch_num].[extension]
// So this function extracts epoch_num, which is in between the last '_' and following '.'
char* get_epoch_from_weightfilename(char *weightfile)
{
	char * copy = malloc(strlen(weightfile) + 1);
	char * orig = copy;
	strcpy(copy, weightfile);
	char* p = strtok( copy, "_" );
	char* temp = NULL;
	while (p) {
		temp = p;
		p = strtok(NULL, "_");
...
{
	char *epoch = get_epoch_from_weightfilename( weightfile );
	printf("Validating %s, which is epoch %s\n", weightfile, epoch);
    int i, j;
    network *net = load_network(filename, weightfile, 0);
    set_batch_network(net, 1);
    srand(time(0));
...
//        show_image(im, "orig", 10000);
//        show_image(crop, "cropped", 10000);
//        cvWaitKey(0);
...
//        printf("%d: top 1: %f, top %d: %f\n", i, avg_acc/(i+1), topk, avg_topk/(i+1));
    ...
    printf("%d: top 1: %f, top %d: %f\n", i, avg_acc/m, topk, avg_topk/m);
    FILE* fp = NULL;
    if ( strstr(datacfg, "train") ) {
    	fp = fopen("logs/training.txt", "a");
    } else {
    	fp = fopen("logs/validation.txt", "a");
    }
    if (fp) {
        fprintf(fp, "%s,%f,%d,%f\n", epoch, avg_acc/m, topk, avg_topk/m);
        fclose(fp);
    }
    free( epoch );
}

// Extracts the basename from the weights file
// Assumes that the file name is .../[basename]_[epoch_num].[extension]
// So this function extracts the part in between the last '/' and following '_' if there is an epoch num. If not, then up to the extension
char* get_basename_from_weightfile(char *weightfile)
{
	char * copy = malloc(strlen(weightfile) + 1);
	char * orig = copy;
	strcpy(copy, weightfile);
...
int get_last_index_to_save(network *net) {
	int i;
	int avgpool = -1;
	int conv_b4_avgpool = -1;

	for (i = 0; i < net->n; i++) {
		layer l = net->layers[i];
		// This only updates if we haven't met the avgpool layer and we're facing a conv layer
		if (avgpool == -1 && (l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL)) {
			conv_b4_avgpool = i;
		}
		// We've met the avgpool layer
		if (l.type == AVGPOOL) {
			avgpool = i;
		}
	}

	// It's always the conv before the avg pool that we care about... right?
	printf("idx of last conv before avg pool: %d\n", conv_b4_avgpool);
	return conv_b4_avgpool + 1;
}

void save_conv_weights(char *filename, char *weightfile)
{
    char *basename = get_basename_from_weightfile(weightfile);
    network *net = load_network(filename, weightfile, 1);

    char targetname[256];
    sprintf(targetname, "weights/%s.conv", basename);

    int last_index = get_last_index_to_save(net);

    if (last_index > 0) {
        save_weights_upto(net, targetname, last_index);
    } else {
    	printf("No convolutional layers to save.\n");
    }
...
void run_classifier(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "save_conv")) save_conv_weights(cfg, weights);
}
